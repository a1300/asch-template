...
const customFormat = printf((info) => {
  const formattedDate = moment(info.timestamp).format('YYYY-MM-DD HH:mm:ss.SSS')
  let level = info.level.toUpperCase()
  const chalkMessageBuild = ['chalk']
  switch (level) {
    case 'SILLY':
  ...
  if (info.meta && info.meta.length > 1) {
    const separated = info.meta.split('.')
    chalkMessageBuild.push(...separated)
  }
  const styles = chalkMessageBuild.join('.')

  const message = styleText(info.message, styles)

  // pass metadata to logger -> logger.info('test', { meta: 'meta' })
  const output = `[${chalk.rgb(30, 144, 255)(formattedDate)}][${level}] ${message}`
  return output
})
...
const dailyRotatefile = new (transports.DailyRotateFile)({
  filename: 'asch-template-%DATE%.log',
  dirname: 'logs',
